...
def phase(
    ...
    mapq: Annotated[int, typer.Option(30, help="Minimum MAPQ to count")] = 30,
    peak_window: Annotated[int, typer.Option(500, help="Peak summit +/- bp window")] = 500,
    anchor_pad: Annotated[int, typer.Option(10_000, help="Anchor padding (bp) when matching mates")] = 10_000,
    min_reads_peak: Annotated[int, typer.Option(5, help="Min total reads to call a peak")] = 5,
    min_pairs_loop: Annotated[int, typer.Option(3, help="Min informative pairs to call a loop")] = 3,
    fdr: Annotated[float, typer.Option(0.05, help="BH-FDR threshold")] = 0.05,
    keep_duplicates: Annotated[bool, typer.Option(False, help="Keep PCR/optical duplicates")] = False,
    validate_loops: Annotated[str, typer.Option("local", help="{none,local} (advanced later)")] = "local",
    config: Annotated[Path | None, typer.Option(None, help="YAML config to override/record params")] = None,
) -> None:
    ...
    # Optional local validation (placeholder)
    if validate_loops == "local":
        from .core.validate_local import run_local_validation  # local import keeps optional dep surface small
