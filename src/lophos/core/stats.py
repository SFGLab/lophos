...
def compute_peak_stats(df_counts: pd.DataFrame) -> pd.DataFrame:
    # expects columns: peak_id, maternal, paternal
    m = df_counts["maternal"].astype(int)
    p = df_counts["paternal"].astype(int)
    total = m + p
    ratio = [ _log2_ratio(int(mi), int(pi)) for mi, pi in zip(m, p) ]
    pvals = [ binomtest(int(mi), int(mi)+int(pi), 0.5, alternative="two-sided").pvalue
              if (mi+pi) > 0 else 1.0 for mi, pi in zip(m, p) ]
    qvals = bh_fdr(pvals)
    out = df_counts.copy()
    out["total"] = total
