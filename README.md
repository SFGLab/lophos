...
**Authorship:** Solely developed by <YOUR NAME>, under the guidance of **Dr. Joanna Borkowska** and **Prof. Dariusz Plewczyński**.
...
2. **Tests** for allele bias with a two-sided **binomial test** at p=0.5.
3. **Adjusts** p-values using **Benjamini–Hochberg (FDR)**.
4. **Calls** the feature as **Maternal / Paternal / Balanced / Undetermined**.

Outputs are plain **BED/BEDPE** tables (easy to sort/filter and load in genome browsers).
...
If your pipeline uses different tokens, we’ll extend the mapping (see src/lophos/constants.py).
...
Default outputs are BED/BEDPE as requested; we can add TSV/CSV/BigBed or track hubs if needed.
...
| `--min_reads_peak`  | Min (M+P) reads to call a peak       | `5`     |
| `--min_pairs_loop`  | Min (M+P) pairs to call a loop       | `3`     |
...
3. Sweep a few parameters to find a sensible setting for your dataset:
...
4. Batch run once parameters look good (match filenames as needed):
...
Test:
M∼Binomial(M+P,p=0.5) (two-sided).
...
Reads are haplotype-tagged via RG. If mates do not share RG, loop counting is conservative; we will add explicit mate lookup soon.
validate-loops local is a placeholder; full local background/Z-score will land in the next releases.
Motif orientation checks for CTCF are stubbed; planned as optional enhancement.

## **Project structure**
  src/lophos/
    cli.py                   # Typer CLI
    core/                    # counts, stats, calls, (placeholder) validation, APA, motif
...
## **Roadmap**
  ...
  Performance profiling & speedups where needed
...
CONTRIBUTING
 — development workflow, style, testing
CODE_OF_CONDUCT
 — expected behavior
...
“Solely developed by Pranjul MIshra, under the guidance of Dr. Joanna Borkowska and Prof. Dariusz Plewczyński with Structural and Functional Genomics Laboratory”
